...
const GridContainer: FC = () => {
    ...
    setGrid((prevGrid) => {
      ...
      switch (resizeDirection) {
        ...
        case "bottom": {
          ...
          if (nextElem !== 0) {
            const nextElemSize = getSizeFromGrid(
              nextElem,
              grid,
              rowCap,
              columnCap,
            ...
            gridPlace(
              id,
              { width: currSize.width, height: currSize.height + 1 },
              { index, row },
              grid,
            );
            gridPlace(
              nextElem,
              nextElemSize,
              gridFindFirst(nextElemSize, grid, rowCap, columnCap),
              grid,
            );
          } else {
            gridPlace(
  ...
  function getCurrPossibleCardSize(id: number): size {
    ...
    const possibleHeight =
      remainInColumn(id, grid, rowCap) + idRowSpanLen(id, grid, rowCap);
