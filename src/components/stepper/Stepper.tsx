            ...
            {Array.from({ length: numberOfSteps }).map((_, index) => (
                <React.Fragment key={index}>
                    <div className="flex items-center">
                        <div className={`w-3.5 h-3.5 rounded-full border ${activeStyles(index)}`}></div>
                        <span className={`ml-2 text-sm ${activeLabels(index)}`}>{labels[index]}</span>
                    </div>
                    {index < numberOfSteps - 1 && <div className={`w-px h-16 ml-1.5 ${activeLines(index)}`} />}
                </React.Fragment>
            ))}
        </div>
    );
